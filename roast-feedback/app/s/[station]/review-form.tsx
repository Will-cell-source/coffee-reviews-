...
function deviceId(): string {
  try {
    ...
    // Private mode or storage blocked — the note still saves, it just
    // counts as a new taster each time.
    return "anon";
  }
}
  ...
  async function send() {
    ...
    try {
      const res = await fetch("/api/review", {
        ...
        body: JSON.stringify({ station, deviceId: deviceId(), text }),
      });
  ...
  if (phase === "sent") {
    return (
      <main className="screen">
        <p className="station">{stationLabel}</p>
        <div className="done">
          <h1>Got it.</h1>
